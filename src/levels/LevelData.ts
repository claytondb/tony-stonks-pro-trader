...
export const LEVELS: LevelData[] = [
  ...
  // =========================================
  {
    ...
    // The cubicle floorplate, floor, walls and suspended ceiling are generated by
    // src/world/OfficeLevel.ts (buildOfficeInterior), which carves a SPINE corridor along Z (|x| < 5.2) and a
    // CROSS corridor along X (|z| < 4.6), both walled with continuous grindable cubicle cap
    // rails, meeting in an open intersection at the spawn. Everything below is staged inside
    // those two corridors so the level reads as connected LINES rather than as a floor plan:
    //   spine  — rail / kicker / rail / fun box, both directions
    //   cross  — kicker gaps (owned by OfficeLevel) and a floor rail in each arm
    objects: [
      // Floor rails running parallel to the spine's cap rails, one lane inboard, so the
      // player can transfer wall -> rail -> wall without touching the carpet.
      { type: 'rail', position: [-4.0, 0, -13.5], rotation: [0, 90, 0], params: { length: 16 } },
      { type: 'rail', position: [4.0, 0, 13.5], rotation: [0, 90, 0], params: { length: 16 } },
      { type: 'rail', position: [4.0, 0, -13.5], rotation: [0, 90, 0], params: { length: 16 } },
      { type: 'rail', position: [-4.0, 0, 13.5], rotation: [0, 90, 0], params: { length: 16 } },

      // A rail in each arm of the cross corridor, offset from the kicker lane.
      { type: 'rail', position: [13.0, 0, -2.6], rotation: [0, 0, 0], params: { length: 11 } },
      { type: 'rail', position: [-13.0, 0, 2.6], rotation: [0, 0, 0], params: { length: 11 } },

      // Kickers facing ALONG the spine (they used to face across it, which meant you could
      // only hit them sideways). Each one has a landing run and a rail to transfer onto.
      { type: 'ramp', position: [-2.4, 0, 8.5], rotation: [0, 0, 0] },
      { type: 'ramp', position: [-2.4, 0, 13.0], rotation: [0, 180, 0] },
      ...
      // Stairs leading down
      { type: 'stairs', position: [0, 0, 20], rotation: [0, 180, 0], params: { steps: 5 } },
    ],

    collectibles: [
      { type: 'document', position: [-4.2, 1, -10], value: 100 },
      { type: 'document', position: [4.2, 1, -10], value: 100 },
      { type: 'document', position: [0, 2, 14], value: 250 },
